...
class OtherStateShippingController extends Controller
{
    protected OtherStateShippingServiceInterface $shippingService;

    public function __construct(OtherStateShippingServiceInterface $shippingService)
    {
        $this->shippingService = $shippingService;
    }

    public function list() {
        $list = OtherStateShipping::orderBy('created_at', 'desc')->simplePaginate(4);
    ...
    public function store(Request $request, Message $message) {
        ...
        $data = $this->shippingService->create($name, $state, $shipping_value, $insurance_value, $registered_value, $express_value);
        if($data) {
    ...
    public function update(Request $request, Message $message, int $id) {
        ...
        $updateData = $this->shippingService->update($id ,$name, $state, $shipping_value, $insurance_value, $registered_value, $express_value);
